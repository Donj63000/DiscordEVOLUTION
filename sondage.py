...
# Stockage provisoire en mémoire : {message_id: {...}}
POLL_STORAGE = {}
...
class SondageCog(commands.Cog):
    """
    Gère la création de sondages (commandes !sondage, !close_sondage)
    et leur clôture automatique ou manuelle.
    """

    def __init__(self, bot: commands.Bot):
        self.bot = bot
        # Tâche périodique : vérification toutes les 20s des sondages arrivés à expiration
    ...
    async def create_sondage(self, ctx: commands.Context, *, args: str = None):
        """
        Crée un sondage et l'envoie sur #annonces (mention @everyone).
        
        Usage :
          !sondage Titre ; Choix1 ; Choix2 ; ... ; temps=JJ:HH:MM
        Exemple :
          !sondage Sortie Donjon ; Bworker ; Ougah ; temps=1:12:30
          -> Fermera dans 1j12h30min
        """
        if not args:
            await ctx.send(
        ...
        for i, part in enumerate(parts):
            if part.lower().startswith("temps="):
                try:
                    raw_time = part.split("=")[1].strip()  # Ex: "1:12:30"
    ...
    async def manual_close_poll(self, ctx: commands.Context, message_id: int = None):
        """
        Ferme manuellement un sondage avec l'ID du message sondage dans #annonces.
        ...
        # (Optionnel) Vérifier que ctx.author.id == poll_data["author_id"] ou role Staff
        # ...
    ...
    async def close_poll(self, message_id: int):
        ...
        # Modifier l'embed original => [Clôturé]
