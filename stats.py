...
class StatsCog(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.stats_enabled = True  # Indicateur si la collecte est activée

        # ---------- Structure agrégée par défaut ----------
        self.stats_data = {
            "messages": {
                "channel_count": {},
                "role_count": {},
        ...
        self.save_loop.start()  # Tâche périodique de sauvegarde

    def cog_unload(self):
        # Annule la tâche planifiée quand le Cog est déchargé
        ...
        if os.path.isfile(DATA_FILE):
            try:
                with open(DATA_FILE, "r", encoding="utf-8") as f:
                    loaded = json.load(f)
                # Fusionner l'existant
                for top_key in self.stats_data.keys():
                    if top_key in loaded:
                        if (
                            isinstance(self.stats_data[top_key], dict)
                            and isinstance(loaded[top_key], dict)
                        ):
    ...
    def reset_stats_data(self):
        """Remet à zéro la structure self.stats_data."""
        self.stats_data = {
            "messages": {
                "channel_count": {},
                "role_count": {},
    ...
    async def on_message(self, message: discord.Message):
        ...
        # Comptabiliser les rôles (sauf @everyone)
    ...
    async def on_message_delete(self, message: discord.Message):
        if not self.stats_enabled:
            return
        # message.author peut être None si c'était un message système
    ...
    # ------------------ Commandes de contrôle des stats ------------------
...
# Pour Discord.py / Py-Cord 2.x : fonction asynchrone d’installation du cog
