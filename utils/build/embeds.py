...
def card(build, report, catalog, durable, *, preview=False):
    ...
    embed.add_field(name="Validation", value=f"{labels[report.equipability]} · {report.completeness} · {report.nature}", inline=False)
    ...
    for row in build.slots:
        ...
        equipment.append(f"{row.slot.replace('_', ' ')} : {safe(item.name, 85)}")
    embed.add_field(name="Équipement", value=("\n".join(equipment) or "Aucun équipement.")[:1024], inline=False)
