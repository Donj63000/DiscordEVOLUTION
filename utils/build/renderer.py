...
def text_report(build, report, catalog, durable):
    rows = [f"EVOLUTION BUILD — {build.name}", f"{build.profile.classe.capitalize()} · niveau {build.profile.level} · révision {build.revision}",
            ...
            f"Équipabilité : {report.equipability} · Données : {report.completeness} · Jets : {report.nature}", ""]
    rows.extend(f"{STAT_LABELS[s]} : {metric_text(report, s)}" for s in STAT_LABELS)
    rows += ["", "ÉQUIPEMENTS"]
    for slot in SLOTS:
        instance = build.in_slot(slot)
        rows.append(f"{slot} : " + (f"{catalog.resolve(instance).name} ({instance.mode})" if instance else "—"))
    ...
    for slot_item in build.slots:
        ...
        rows.extend(f"  {e.ref}: {e.kind} {e.stat or e.element or '?'} [{e.low}, {e.high}] {e.text}" for e in template.effects)
...
def render(build, report, catalog, durable, icons=None):
    ...
    draw.text((40, 201), "PostgreSQL · privé" if durable else "ESSAI NON DURABLE — export JSON conseillé", font=small, fill="#ffcd83")
        ...
        if instance:
            ...
            if raw and len(raw) <= 1024 * 1024:
                try:
                    ...
                    pass
        draw.text((x + 68, y + 6), slot.replace("_", " ").capitalize(), font=small, fill="#8dbdf8")
    ...
    draw.text((40, 1330), f"Jets : {report.nature} · Catalogue {build.catalog_id[:14]} · Règles {build.rules_id[:14]}", font=small, fill="#b9cce3")
