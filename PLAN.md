Voici la modification propre à appliquer. Le cœur du changement est de remplacer complètement `up.py` : il n’y aura plus aucune campagne automatique, plus aucun vote, plus aucun message automatique dans `#general-staff`. Le module ne fera désormais quelque chose que lorsqu’un membre du staff lance `!veteran`.

Remplace intégralement le contenu de `up.py` par ce code :
...
Dans `help.py`, remplace uniquement le texte de la commande `!veteran`.
...
Dans `.env.example`, remplace les anciennes variables liées aux votes automatiques :

```env
UP_SCAN_DAYS=180
UP_SCAN_LIMIT_PER_CHANNEL=5000
UP_PROMOTION_CAMPAIGN_DAY=15
UP_PROMOTION_CAMPAIGN_HOUR=20
UP_PROMOTION_CAMPAIGN_TIMEZONE=Europe/Paris
UP_PROMOTION_VOTE_DURATION_SECONDS=3600
```
...
UP_SCAN_DAYS=180
UP_SCAN_LIMIT_PER_CHANNEL=5000
UP_SCAN_DELAY_SECONDS=0.2
UP_SCAN_RETRIES=2
UP_VETERAN_MESSAGE_THRESHOLD=20
UP_VETERAN_JOINED_THRESHOLD_DAYS=180
UP_VETERAN_CANDIDATES_PER_PAGE=10
UP_VETERAN_VIEW_TIMEOUT_SECONDS=900
```

Et ajoute, près de `ANNONCE_CHANNEL_NAME=annonces`, cette variable optionnelle :

```env
ANNONCE_CHANNEL_ID=
```

Tu peux garder `ANNONCE_CHANNEL_NAME=annonces`. Si ton salon a un nom avec emoji ou décoration, le plus fiable en production est de renseigner directement `ANNONCE_CHANNEL_ID`.

À supprimer aussi de ton `.env` réel sur Render ou ton hébergement si elles existent encore :

```env
UP_PROMOTION_AUTO_VOTES
UP_PROMOTION_CAMPAIGN_DAY
UP_PROMOTION_CAMPAIGN_HOUR
UP_PROMOTION_CAMPAIGN_TIMEZONE
UP_PROMOTION_CAMPAIGN_TZ
UP_PROMOTION_VOTE_DURATION_SECONDS
```

Avec ce remplacement, le bot ne lance plus aucune tâche mensuelle, ne planifie plus aucune finalisation de vote, ne poste plus rien automatiquement dans `#general-staff`, et ne crée plus aucune réaction ✅/❌. L’ancien état `voting` éventuellement présent dans `promotions_data.json` est nettoyé silencieusement au chargement, sans message staff.

Hypothèses conservées depuis ton système actuel : un membre est éligible s’il n’est pas bot, possède le rôle `Membre validé d'Evolution`, ne possède pas `Invité`, ne possède pas déjà `Vétéran`, a au moins `180` jours d’ancienneté et au moins `20` messages trouvés pendant le scan. Ces seuils sont maintenant configurables via `.env`.

Permissions Discord nécessaires : le bot doit avoir l’intent membres activé, la permission `Gérer les rôles`, son rôle doit être placé au-dessus du rôle `Vétéran`, et il doit pouvoir envoyer des messages dans `#annonces`.

Je n’ai pas pu exécuter `pytest` dans l’environnement ici parce que le module `discord` n’est pas installé, mais le fichier `up.py` compile syntaxiquement. Après installation des dépendances du projet, lance :

```bash
python -m pytest tests/test_up_scan.py
python -m pytest
```
