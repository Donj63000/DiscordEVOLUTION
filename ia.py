...
class IAStaff(commands.Cog):
    ...
    async def iastaff(self, ctx, *, prompt: str):
        ...
        for i, chunk in enumerate(chunks, 1):
            ...
            footer_model = self.model if self.prompt_id is None else f"prompt {self.prompt_id} v{self.prompt_version}"
            emb.set_footer(text=f"Modèle: {footer_model}")
